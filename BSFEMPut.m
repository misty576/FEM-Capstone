...
x_min = log(25/K); x_max = log(500/K);
...
IC = @(x, k1) max(0, exp(0.5 * x * (k1 - 1)) - exp(0.5 * x * (k1 + 1)));

% Boundary conditions (alpha - value at x_min , beta - value at x_max)
alpha = @(tau) exp(1/2*(k1-1)*x_max + 1/4*tau*(k1-1)^2);
beta = @(tau) 0;
...
s.LineWidth = 0.75;
colormap turbo
title('Black-Scholes FEM Approximation for European Put');
...
title('European Put Option Prices at Different Times');
